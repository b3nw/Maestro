 ...
 * Used as fallback when 'which' fails over SSH (e.g., non-login shells
 * that don't have full PATH configured).
 */
function getSshKnownPaths(binaryName: string): string[] {
	...
	switch (binaryName) {
		...
		case 'gemini':
			return ['$HOME/.local/bin/gemini', '/opt/homebrew/bin/gemini', '/usr/local/bin/gemini'];
		default:
			return [];
	}
}
 ...
 * Falls back to probing known installation paths when 'which' fails.
